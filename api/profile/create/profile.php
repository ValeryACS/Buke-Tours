...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if (!$stmt) {
                throw new Exception("Error al preparar INSERT customer: " . $mysqli->error);
            }
            ...
            if (!$stmt->execute()) {
                throw new Exception($stmt->error ?: "Error al insertar customer");
            }
    ...
    $data = [
        ...
        'message' => empty($errors) ? 'El Usuario ha sido Creado exitOsamente' : $errors[0] ?? 'Error al crear Usuario',
